...
const Search = () => {
    ...
    function search() {
        const query = document.getElementById("query_search").value;
        console.log(query)
        if (query) {
            ...
            <div className="search__container">
                <div className="search__container__items">
                    <div className="search__container__items__input">
                        <input type="text" placeholder="ПОИСК..." id="query_search" />
                        <img src={Zoom} alt="" onClick={() => search()} />
                    </div>
                    <div className="search__container__items__weather">
                        {(homedetails.icon !== "01d") &
                        (homedetails.icon !== "01n") ? (
                            <img
                        ...
                        <div className="weather__box" id="weather__box">
                            <span id="morn">
                                MONR :{" "}
                                {homedetails.morn_temp < 0
                                    ? homedetails.morn_temp.slice(0, 3)
                                    : `+${homedetails.morn_temp}`.slice(
                                          0,
                                          3
                                      )}{" "}
                            </span>
                            <span id="even">
                                EVEN :{" "}
                                {homedetails.eve_temp < 0
                                    ? homedetails.eve_temp.slice(0, 3)
                                    : `+${homedetails.eve_temp}`.slice(
                                          0,
                                          3
                                      )}{" "}
                            </span>
                            <span id="nigh">
                                NIGH :{" "}
                                {homedetails.night_temp < 0
                                    ? homedetails.night_temp.slice(0, 3)
                                    : `+${homedetails.night_temp}`.slice(
                                          0,
                                          3
                                      )}{" "}
                            </span>
                    ...
                    <div className="search__container__items__currency">
                        ...
                        />
                        <div
                            className="other__currency__box"
                            id="other__currency__box"
                        >
                            <span id="currency_rub">
                                RUB : {homedetails.RUB}{" "}
                            </span>
                            <span id="currency_eur">
                                EURO : {homedetails.EUR}{" "}
                            </span>
                        </div>
                    </div>
                </div>
